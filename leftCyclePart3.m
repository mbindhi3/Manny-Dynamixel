...
a16d = deriv(a16);
ParamList = {};
a3InitialConstraint =  constraint(-0.9730,'a3',-0.9730,'initial');
a4InitialConstraint = constraint(  1.3242 ,'a4',  1.3242 ,'initial');
a5InitialConstraint = constraint(-0.3512,'a5',-0.3512,'initial');
a14InitialConstraint =  constraint(-0.6728,'a14', -0.6728 ,'initial');
a15InitialConstraint = constraint( 1.4882 , 'a15', 1.4882 ,'initial');
a16InitialConstraint = constraint( -0.8154 ,'a16', -0.8154,'initial');
a14FinalConstraint =  constraint(-0.0637,'a14',-0.0637,'final');
a15FinalConstraint = constraint(0.7513, 'a15',0.7513,'final');
a16FinalConstraint = constraint(-0.6876 ,'a16',-0.6876 ,'final');
a3FinalConstraint =  constraint(-0.5853, 'a3',-0.5853,'final');
a4FinalConstraint =  constraint(1.1022, 'a4',1.1022,'final');
a5FinalConstraint = constraint(-0.5169,'a5',-0.5169,'final');
zLeftFootBasePositionTrajectoryConstraint = constraint(0,zLeftFootBaseEquation,0,'trajectory');
xLeftFootBasePositionTrajectoryConstraint = constraint(2,xLeftFootBaseEquation,2,'trajectory');
orientationLeftFootTrajectoryConstraint = constraint(0,'a3 + a4 + a5',0,'trajectory');
orientationRightFootTrajectoryConstraint = constraint(0,'a14 + a15 + a16',0,'trajectory');
xCOMTrajectoryConstraint = constraint(1,xCoMwrtRightFootEquation,3.2,'trajectory');
xCOMFinalConstraint = constraint(2.8,xCoMwrtRightFootEquation,3.2,'final');
trajectoryConstraints = xCOMTrajectoryConstraint + orientationLeftFootTrajectoryConstraint + orientationRightFootTrajectoryConstraint;
...
finalConstraints =  a3FinalConstraint + a4FinalConstraint + a5FinalConstraint + a14FinalConstraint + a15FinalConstraint + a16FinalConstraint;
...
leftCyclePart3Alphas = [a3;a4;a5;a14;a15;a16];
save('leftCyclePart3Alphas','leftCyclePart3Alphas')