...
a16d = deriv(a16);
ParamList = {};
a3InitialConstraint = constraint(-0.3224,'a3',-0.3224,'initial');
a4InitialConstraint = constraint(0.9063,'a4',0.9063,'initial');
a5InitialConstraint = constraint(-0.5839,'a5',-0.5839,'initial');
a14InitialConstraint = constraint(-0.6068,'a14',-0.6068,'initial');
a15InitialConstraint = constraint(0.7869,'a15',0.7869,'initial');
a16InitialConstraint = constraint(-0.1802,'a16',-0.1802,'initial');
a3TrajectoryConstraint = constraint(-3*pi/4,'a3',pi/4,'trajectory');
a4TrajectoryConstraint = constraint(-pi/4,'a4',pi/2,'trajectory');
a5TrajectoryConstraint = constraint(-pi/3,'a5',pi/4,'trajectory');
a14TrajectoryConstraint = constraint(-3*pi/4,'a14',pi/2,'trajectory');
a15TrajectoryConstraint = constraint(-pi/2,'a15',pi/2,'trajectory');
a16TrajectoryConstraint = constraint(-pi/3,'a16',pi/2,'trajectory');
orientationFinalConstraint = constraint(0,'a16 + a14 + a15',0,'final');
...
xCOMFinalConstraint = constraint(2,'(3*89089168577^(1/2)*cos(a4 + a5 + atan(1462372/297821)))/350680 - (22348350232498^(1/2)*cos(a5 - atan(4642523/891813)))/1753400 - (483284377^(1/2)*cos(a14 - a4 - a5 - a3 + a15 + a16 + atan(19684/9789)))/1753400 + (4528831224905^(1/2)*cos(a3 + a4 + a5 + atan(2060011/534028)))/876700 - (27*32969569^(1/2)*cos(a3 + a4 + a5 - a14 + atan(5688/785)))/350680 + (123*19333106^(1/2)*cos(a3 + a4 + a5 - a14 - a15 - atan(4309/875)))/1753400 - 588411/1753400',3,'final');
...
trajectoryConstraints = a3TrajectoryConstraint + a4TrajectoryConstraint + a5TrajectoryConstraint + a14TrajectoryConstraint + a15TrajectoryConstraint + a16TrajectoryConstraint + orientationTrajectoryConstraint;
finalConstraints =  xCOMFinalConstraint + xRightFootBasePositionFinalConstraint + zRightFootBasePositionFinalConstraint + orientationFinalConstraint;
...
preCyclePart3Alphas = [a3;a4;a5;a14;a15;a16];
save('preCyclePart3Alphas','preCyclePart3Alphas')